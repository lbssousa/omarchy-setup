...
| `playbooks/capslock.yml` | Caps Lock via keyd — tap=Esc, hold=Ctrl, Shift+CapsLock=CapsLock literal; move o Compose do Omarchy do Caps Lock pra tecla Menu (tag `capslock`) |
...
| `Justfile`                  | Atalhos (`just setup`, `just nvidia`, `just ptbr`, `just bitwarden`, `just podman`, `just distrobox`, `just libfprint`, `just printer`, `just hypr-scrolling-resize`, `just text-size`, `just nightlight-solar`, `just capslock`, `just sudo`, `just polkit`, `just secureboot`, `just gpg-yubikey`, `just ssh-yubikey`) |
