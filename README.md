...
| Brave | `brave` | Installs Brave from Flathub, replacing the AUR `brave-origin-bin` package (uninstalled) that used to drive the Brave PWA web app swap (removed — see git history). Bootstraps Flatpak + the Flathub remote itself. Also removes the leftover `/etc/brave/policies/managed/omarchy-setup-webapps.json` and old `WhatsApp`/`Microsoft Teams`/`YouTube` launcher files that swap wrote, plus any Omarchy-seeded web app launcher (e.g. `Google Maps`) superseded by a container-aware Brave PWA (`brave_pwa_containers`). |
